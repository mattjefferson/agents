...
def _default_out_dir() -> str:
    ...
    return os.path.join(tmp_root, f"image-openai-{_stamp()}")
...
def _fetch_image_bytes(url: str, timeout_s: int) -> bytes:
    try:
        with urllib.request.urlopen(url, timeout=timeout_s) as resp:
            return resp.read()
    except Exception as e:
        raise SystemExit(f"image download failed: {e}")
...
def main(argv: list[str]) -> int:
    ...
    for i, prompt in enumerate(prompts, 1):
        ...
        if b64:
            ...
            png = _fetch_image_bytes(entry["url"], timeout_s=args.timeout)
        else:
