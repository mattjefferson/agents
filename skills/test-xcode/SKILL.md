...
- Create P1 task for each build error
...
4. **If "Create task":**
   - Route through `/tasks`:
     - `/tasks detect`
     - `/tasks create --title "Xcode test: {description}" --type bug --priority P1 --description "..."`
     - `/tasks update --id <id> --notes "Simulator steps + logs + crash details" --acceptance-criteria "- [ ] Crash fixed"`
   - Continue testing
...
### Created Tasks: [count]
- `{task-id}` - Xcode test failure task (created via `/tasks`)
...
When reviewing PRs that touch iOS code, the `/review-plan` command can spawn this as a subagent:

```
Task general-purpose("Run /test-xcode for scheme [name]. Build, install on simulator, test key screens, check for crashes.")
```
